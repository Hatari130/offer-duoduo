...
export function isCapturePositionRejected(value?: string): boolean {
  ...
  // occupation token 实习生 but is a navigation label, not a position.
  if (CAPTURE_CAMPAIGN_PATTERN.test(candidate)) return true;
  if (CAPTURE_OCCUPATION_PATTERN.test(candidate)) return false;
  return CAPTURE_PROCESS_PATTERN.test(candidate);
}
...
function progressLabel(evidence: NonNullable<ExtractedJob["progressEvidence"]>[number]) {
  ...
  const raw =
    evidence.terminalStatus?.trim() ||
    evidence.currentStage?.trim() ||
    reached?.label.trim();
  return normalizeExternalStage(raw) || raw;
}

const EXTERNAL_STAGE_PREFIX_PATTERN = /^(?:当前进度|当前状态|应聘进度|投递进度|招聘进度|申请进度)[：:]\s*/i;
 ...
 */
export function normalizeExternalStage(value?: string): string | undefined {
  const stage = (value || "")
    .trim()
    .replace(EXTERNAL_STAGE_PREFIX_PATTERN, "")
    .toLowerCase();
  if (!stage) return undefined;
  if (/未通过|不通过|不合适|淘汰|流程终止|流程结束|已结束|拒绝|未录用|已撤回|已终止/.test(stage)) {
    return "已结束";
  }
  if (/offer|录用|意向书|背调|背景调查|体检|签约|审批|发放/.test(stage)) return "Offer";
  if (/面试|一面|二面|三面|hr面|群面|终面|复试|业务面|主管面/.test(stage)) return "面试";
  if (/笔试|测评|机考|在线测试/.test(stage)) return "笔试";
  if (/简历筛选|简历初筛|初筛|复筛|筛选|简历评估|简历审核|资格审核|简历处理中/.test(stage)) {
    return "简历初筛";
  }
