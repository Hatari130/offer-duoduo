        ...
        <div className="resume-editor-content">
          ...
          <div className="resume-editor-section-list">
            ...
            <EditorSection
              keyName="internships"
              title="实习经历"
              description={`${internshipExperiences.length} 段实习 · 自动匹配网申的实习经历区块`}
            ...
            <EditorSection
              keyName="work"
              title="工作经历"
              description={`${workExperiences.length} 段工作 · 自动匹配网申的工作经历区块`}
              icon={<BriefcaseBusiness size={17} />}
                ...
                return (
                  ...
                  >
                    <div className="resume-editor-grid">
                      ...
                      <EditorField label="工作岗位"><input value={item.title} onChange={(event) => updateExperience(item.id, { title: event.target.value })} /></EditorField>
                      <EditorField label="开始时间"><input type="month" value={monthInputValue(item.startDate)} onChange={(event) => updateExperience(item.id, { startDate: event.target.value })} /></EditorField>
                      <EditorField label="结束时间"><input type="month" value={monthInputValue(item.endDate)} onChange={(event) => updateExperience(item.id, { endDate: event.target.value })} /></EditorField>
                      <EditorField label="工作职责" wide><textarea rows={5} value={item.description} onChange={(event) => updateExperience(item.id, { description: event.target.value })} placeholder="写清楚负责内容、方法和结果" /></EditorField>
                    </div>
                  </EditorEntry>
                );
              })}
            ...
            >
              ...
              {draft.projects.length === 0 && <EmptyEditorEntry text="还没有项目经历，点击添加" onClick={() => {
                const projId = newId("project");
                set("projects", [{ id: projId, name: "", role: "", startDate: "", endDate: "", description: "" }]);
                setExpandedEntries((curr) => new Set(curr).add(projId));
              }} />}
            </EditorSection>
            ...
            <EditorSection
              keyName="campus"
              title="在校经历"
              description={`${draft.campusExperiences.length} 段经历 · 社团、学生组织或志愿服务`}
            ...
            <EditorSection
              keyName="awards"
              title="获奖与证书"
              description={`${draft.awards.length} 项奖励 · 可补充名称、等级与详情`}
              icon={<Trophy size={17} />}
              ...
              toggleAllAction={draft.awards.length > 0 ? (draft.awards.every((e) => expandedEntries.has(e.id)) ? "全部收起" : "全部展开") : undefined}
              onToggleAll={() => toggleAllEntriesInSection(draft.awards.map((e) => e.id))}
            >
              ...
              {draft.awards.length === 0 && <EmptyEditorEntry text="还没有获奖记录，点击添加" onClick={() => {
                const awardId = newId("award");
                set("awards", [{ id: awardId, date: "", name: "", level: "", description: "" }]);
                setExpandedEntries((curr) => new Set(curr).add(awardId));
              }} />}
            </EditorSection>
            ...
            <EditorSection
              keyName="languages"
              title="外语能力"
              description={`${draft.languages?.length || 0} 项语言资料 · 支持网申逐项填写`}
              icon={<CircleUserRound size={17} />}
              open={openSections.has("languages")}
              onToggle={toggle}
              action="添加外语能力"
              onAction={() => addProfileRecord("languages", { id: newId("language"), name: "", certificate: "", englishLevel: "", score: "", proficiency: "", listeningSpeaking: "", readingWriting: "" })}
              toggleAllAction={(draft.languages?.length || 0) > 0 ? ((draft.languages || []).every((_, i) => expandedEntries.has(`language-${i}`)) ? "全部收起" : "全部展开") : undefined}
              onToggleAll={() => toggleAllEntriesInSection((draft.languages || []).map((item, i) => item.id || `language-${i}`))}
            >
            ...
            <EditorSection
              ...
              title="证书资格"
              description={`${draft.qualifications?.length || 0} 项证书 · 与奖项分开维护`}
              icon={<Trophy size={17} />}
              open={openSections.has("qualifications")}
              onToggle={toggle}
              action="添加证书"
              onAction={() => addProfileRecord("qualifications", { id: newId("qualification"), date: "", name: "", number: "", description: "" })}
              toggleAllAction={(draft.qualifications?.length || 0) > 0 ? ((draft.qualifications || []).every((_, i) => expandedEntries.has(`qualification-${i}`)) ? "全部收起" : "全部展开") : undefined}
              onToggleAll={() => toggleAllEntriesInSection((draft.qualifications || []).map((item, i) => item.id || `qualification-${i}`))}
            >
            ...
            <EditorSection
              keyName="family"
              title="家庭情况"
              description={`${draft.familyMembers?.length || 0} 位家庭成员 · 仅用于要求该信息的网申`}
              icon={<CircleUserRound size={17} />}
              open={openSections.has("family")}
              onToggle={toggle}
              action="添加家庭成员"
              onAction={() => addProfileRecord("familyMembers", { id: newId("family"), name: "", relation: "", phone: "", company: "", position: "", politicalStatus: "" })}
              toggleAllAction={(draft.familyMembers?.length || 0) > 0 ? ((draft.familyMembers || []).every((_, i) => expandedEntries.has(`family-${i}`)) ? "全部收起" : "全部展开") : undefined}
              onToggleAll={() => toggleAllEntriesInSection((draft.familyMembers || []).map((item, i) => item.id || `family-${i}`))}
            >
