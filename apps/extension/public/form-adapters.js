(() => {
  ...
  const commonMappings = [
    ...
    ["currentCity", "现居城市|当前城市|所在城市|居住地|现居地|current.?city|current.?location"],
    ...
    ["workYears", "工作年限|工作经验年限|从业年限|work.?years?"],
    ...
    ["selfIntroduction", "自我介绍|个人简介|个人总结|自我评价|about.?you|self.?intro|profile"],
  ...
  const adapters = [
    {
      ...
      mappings: [
        ...
        ["major", "专业名称|所学专业"],
        ["degree", "学历|学位"],
        ["recruitmentType", "是否统招|统招"],
        ["graduateStatus", "应届.?往届|应届生|往届生"],
        ["graduationDate", "毕业时间|毕业日期"],
        ["nativePlace", "籍贯|户籍"],
        ["height", "身高"],
        ["weight", "体重"]
      ]
    },
    {
      ...
      markers: ["[data-field]", "[data-question]", "[class*='moka']", "[class*='form-item']"],
      mappings: [
  ...
  adapters.forEach((adapter) => {
    adapter.compiled = compiled(commonMappings.concat(adapter.mappings || []));
  });
  ...
  const match = (element, label, adapter = resolve()) => {
    ...
    const candidates = [...(adapter.compiled || []), ...(overrides.get(adapter.id) || [])];
    for (const rule of candidates) {
      if (rule.pattern.test(text)) {
        return {
          key: rule.key,
          confidence: adapter.id === "generic" ? 0.86 : 0.96,
          source: "rules",
