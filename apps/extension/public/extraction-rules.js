(() => {
  ...
  const nonPositionPattern = /^(?:职位|岗位|职位名称|岗位名称|所属部门|申请日期|申请编号|投递时间|申请时间|工作地点|城市|状态|详情|操作|刷新活跃度|网申投递|专场招聘会投递|我的简历)$/i;
  const metadataPattern = /^(?:20\d{2}[年./-]\d{1,2}[月./-]\d{1,2}日?|\d{8,16}|[A-Z]\d{5,})$/i;
  const campaignPattern = /(?:校园招聘|社会招聘|招聘官网|招聘平台|招聘计划|实习生计划|管培计划|专项计划|专场招聘|招聘项目|\d{4}届(?:实习生|应届生|校招生)|JD\s*YOUNG)/i;
