              ...
              {filtered.map((item) => {
                ...
                return (
                  <tr key={application.id} className={selectableStage(application.stage) === "closed" ? "application-row--closed" : undefined}>
                    ...
                    <td data-label="岗位 JD"><button className={`application-jd-entry ${application.rawExcerpt?.trim() ? "has-content" : ""}`} type="button" aria-label={`${application.rawExcerpt?.trim() ? "查看" : "为"} ${application.company} ${application.position} ${application.rawExcerpt?.trim() ? "的岗位 JD" : "添加岗位 JD"}`} onClick={() => setDialog({ mode: "edit", item })}><FileText aria-hidden="true" size={13} /><span>{application.rawExcerpt?.trim() ? "查看" : "添加"}</span></button></td>
                  ...
                  {stageItems.map((item) => {
                    ...
                    return (
                      ...
                      <div className="board-card-actions">
                        <button className={`board-jd-entry ${item.application.rawExcerpt?.trim() ? "has-content" : ""}`} type="button" aria-label={`${item.application.rawExcerpt?.trim() ? "查看" : "为"} ${item.application.company} ${item.application.position} ${item.application.rawExcerpt?.trim() ? "的岗位 JD" : "添加岗位 JD"}`} onClick={() => setDialog({ mode: "edit", item })}><FileText aria-hidden="true" size={13} /><span>{item.application.rawExcerpt?.trim() ? "查看" : "添加"}</span></button>
...
function ApplicationDialog({
  ...
  const [appliedAt, setAppliedAt] = useState(
    item?.application.appliedAt ? item.application.appliedAt.slice(0, 10) : ""
  );
